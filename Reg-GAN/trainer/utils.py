...
class Normalize(object):
    ...
    def normalize(self, arr):
        if isinstance(arr, Image.Image):
            arr = np.array(arr)
        if len(arr.shape) == 3 and arr.shape[0] != self.output_nc:
            arr = arr.transpose((2, 0, 1))
        return cv2.normalize(arr, None, -1, 1, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_32F)
...
def tensor2image(tensor,):
    ...
    return image1[2,:,:].astype(np.uint8)
